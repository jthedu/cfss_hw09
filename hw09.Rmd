...
```{r}
# ORIGINAL - RUN THIS TO GET ORIGINAL combined_word df
...
# could delete this if end up using stems
...
not affected by stemming - probably better if don't stem
...
mail_rec <- recipe(~., data = combined_word) %>%
  ...
  step_ngram(word, num_tokens = 1, min_num_tokens = 1) %>%
  step_tokenfilter(word, max_tokens = 1500) %>%
  step_tf(word)
...
top_terms_lda4 <- mail_lda4 %>%
  tidy() %>%
  group_by(topic) %>%
  slice_max(n = 8, order_by = beta, with_ties = FALSE) %>% 
